...
export class A11yAware {
  ...
  getControlledElement () {
    ...
    this.assert(controlledElem, this._t('The `aria-controls` attribute should point to a valid `id`.'));
  ...
  assert (assertion, msgFail, msgOK) {
    ...
    /* if (this._messageCallback) {
      this._messageCallback({ assertion, msgFail, msgOK });
    } /* else {
      console.debug('A11yAware: assert - else');
      this._messages.push({ assertion, msgFail, msgOK });
    } */
  }

  setMessageCallback (callbackFunc) {
    console.assert(callbackFunc);
    console.debug('AllyAware: setMessageCallback', callbackFunc);
    this._messageCallback = callbackFunc;
  }
