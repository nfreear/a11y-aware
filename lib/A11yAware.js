 ...
 *
 * @event MessageEvent#a11y_aware:assert
 *
...
export class A11yAware {
  ...
  testHasPopupAttribute () {
    this.assert(this.element.hasAttribute('aria-haspopup'), this._t('Expecting an `aria-haspopup` attribute on trigger button.'));
    this.assert(this.isValidHasPopup, this._t('Expecting `aria-haspopup` to be one of the defined values.'));
  }
  ...
  get isValidHasPopup () {
    const hasPopup = this.element.getAttribute('aria-haspopup');
    return /^(true|menu|listbox|tree|grid|dialog)$/.test(hasPopup);
  }

  /**
   * @fires MessageEvent#a11y_aware:assert
   */
  assert (assertion, msgFail, msgOK) {
    console.assert(!!assertion, msgFail, null);

    this._channel.postMessage({ type: 'assert', idx: this.idx, assertion: !!assertion, msgFail, msgOK });
  }
